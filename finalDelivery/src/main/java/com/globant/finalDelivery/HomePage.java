...
public class HomePage {
	...
	public static final String[] AFE_OPTIONS = new String []{"Senior (65 years and older)","Adult (25 - 64 years)","Young (18 - 24 years)","Young (12 - 17 years)","Child (2 - 11 years)"};
	
	 public void searchFlighRoundTrip(String origin, String destination,String depatureYear,String departureMonth,String departureDay,String backYear,
			String backMoth,String backDay, String[] passengers){
		...
		if(Integer.valueOf(depatureYear) == calendar.get(Calendar.YEAR) && Integer.valueOf(departureMonth) < calendar.get(Calendar.MONTH)){
			throw new IllegalStateException("Use a valid date in the future");
		}
		if(Integer.valueOf(depatureYear) == calendar.get(Calendar.YEAR) && Integer.valueOf(departureMonth) == calendar.get(Calendar.MONTH) && Integer.valueOf(departureDay) < calendar.get(Calendar.DAY_OF_MONTH)){
			throw new IllegalStateException("Use a valid date in the future");
		}
		...
		sleep(6);
		WebElement nextMonthButton = HomePageTest.driver.findElement(By.xpath("//*[@id='calendar']/header/div[2]/button[2]"));
		List<WebElement> date = HomePageTest.driver.findElements(By.id("calendar--"+ depatureYear+departureMonth+departureDay));
		sleep(1);
		...
		while(clicked){
			if(date.size()==0){
				...
				sleep(1);
				date = HomePageTest.driver.findElements(By.id("calendar--"+ depatureYear+departureMonth+departureDay));
			}else{
				try{
					date.get(0).click();
					clicked = false;
		...
		List<WebElement> backDate = HomePageTest.driver.findElements(By.id("calendar--"+ backYear+backMoth+backDay));
		clicked = true;
		while(clicked){
			if(backDate.size() == 0){
				nextMonthButton.click();
	...
	public void searchOneWay(String origin,String destination,String depatureYear,String departureMonth,String departureDay,String[] passengers){
		oneWay.click();
		originInput.sendKeys(origin);
		sleep(5);
		destinationInput.sendKeys(destination);
